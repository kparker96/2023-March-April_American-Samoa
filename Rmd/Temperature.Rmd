...
# Fagatele Clean Stats
```{r}
model <- lmer(Temperature ~ Depth + (1 | Date), data = Fagatele_clean)
summary(model)
...
```{r}
Fagatele_daily <- Fagatele_clean %>%
  group_by(Date, Depth) %>%
  summarise(Temperature = mean(Temperature, na.rm = TRUE), .groups = "drop")

Fagatele_wide <- Fagatele_daily %>%
  pivot_wider(names_from = Depth, values_from = Temperature)

library(lmerTest)

model <- lmer(Temperature ~ Depth + (1 | Date), data = Fagatele_daily)

summary(model)

library(emmeans)

emmeans(model, pairwise ~ Depth)
```
