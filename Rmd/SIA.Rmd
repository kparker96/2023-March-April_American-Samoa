...
ggplot(sia_df_clean %>% filter(Type == "Host"), aes(x = Depth, y = mean_CN_ratio, fill = Depth)) +
  geom_boxplot() +
  facet_wrap(~Type) +
...
leveneTest(mean_CN_ratio ~ Depth, data = sia_df_clean_host)

kruskal.test(mean_CN_ratio ~ Depth, data = sia_df_clean_host)
...
leveneTest(mean_CN_ratio ~ Depth, data = sia_df_clean_zoox)
