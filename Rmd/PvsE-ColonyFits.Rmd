...
write_xlsx(results_df, path = "data/respirometry/PvsI_colony_parameters.xlsx")
...
pe_table_formatted <- pe_table %>%
  mutate(
    `Pmax (µmol O2 cm⁻² min⁻¹)` =
      sprintf("%.3f ± %.3f", Pmax_mean, Pmax_se),

    `α` =
      sprintf("%.4f ± %.4f", alpha_mean, alpha_se),

    `Ek (µmol quanta m⁻² s⁻¹)` =
      sprintf("%.1f ± %.1f", Ek_mean, Ek_se)
  ) %>%
  select(depth,
         `Pmax (µmol O2 cm⁻² min⁻¹)`,
         `α`,
