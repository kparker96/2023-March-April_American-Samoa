...
t.test(ED50 ~ Site_Genus, data = Geno_ED50s_long)
...
```{r preddata}
Montipora_shallow_preddata = data.frame(temp = seq(27,43, length.out = 100))
...
Montipora_deep_preddata = data.frame(temp = seq(27,43, length.out = 100))
...
ggplot() +
  geom_point(data = PAM_data, aes(x = Temperature, y = PAM, color = Depth), size = 1, width = 0.25) +
  scale_x_continuous(limits=c(28.5,41.5), breaks=c(29,31,33,35,37,39,41), expand = c(-0.01,0)) +
  scale_y_continuous(limits=c(-0.005, 0.72), expand = c(0,0)) +
  
# Shallow
  geom_line(data = Montipora_shallow_preddata, aes(x = temp, y = fvfm), color = '#3B82F6', 
            show.legend = FALSE, size = 1) +
  geom_ribbon(data = Montipora_shallow_preddata, aes(x = temp, ymin=Lower, ymax=Upper), fill = '#3B82F6', 
              linetype=2, alpha = 0.2, size = 0.5) +
  geom_vline(data = Coeff_means, aes(xintercept = Montipora_shallow_coeff_mean), color = '#3B82F6',
             show.legend = FALSE, size = 1) +
  annotate("rect", xmin=Coeff_lowers$Montipora_shallow_coeff_lower,
           xmax=Coeff_uppers$Montipora_shallow_coeff_upper, ymin=-Inf, ymax=Inf, fill= '#3B82F6',  
           alpha = 0.1) +
  geom_text(data = Coeff_means, aes(label=paste0(round(Montipora_shallow_coeff_mean, digits = 2), " °C")),
            x = 30, y = 0.02, show.legend = FALSE, color = '#3B82F6', fontface = 'bold') +
  
# Deep
  geom_line(data = Montipora_deep_preddata, aes(x = temp, y = fvfm), color = '#0A2472', show.legend = FALSE,
            size = 1) +
  geom_ribbon(data = Montipora_deep_preddata, aes(x = temp, ymin=Lower, ymax=Upper), fill = '#0A2472',
              linetype=2, alpha = 0.2, size = 0.5) +
  geom_vline(data = Coeff_means, aes(xintercept = Montipora_deep_coeff_mean), color = '#0A2472', 
             show.legend = FALSE, size = 1) +
  annotate("rect", xmin=Coeff_lowers$Montipora_deep_coeff_lower, xmax=Coeff_uppers$Montipora_deep_coeff_upper,
           ymin=-Inf, ymax=Inf, fill= '#0A2472',  alpha = 0.1) +
  geom_text(data = Coeff_means, aes(label=paste0(round(Montipora_deep_coeff_mean, digits = 2), " °C")),
            x = 29.8, y = 0.0, show.legend = FALSE, color = '#0A2472', fontface = 'bold') +
  ...
  scale_color_manual(values=c('#0A2472','#3B82F6')) +
  ylab(bquote(F[v]/F[m]))+ 
  xlab("Temperature (°C)")
