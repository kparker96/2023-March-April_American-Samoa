...
# Phytotools Loop - Individual Curves
```{r}
...
# Phytotools Loop - Depth Level Fits
```{r}
# Average genofits for population fit
rlc.data.pop <- rlc.data %>%
  group_by(id, par) %>%
  dplyr::summarize(pop_etr = mean(etr, na.rm = TRUE), .groups = 'drop')
...
#### Plot averaged data with SD and fitted model ####
target_par <- c(0, 25, 45, 66, 91, 126, 191, 286, 422, 632, 825, 1155, 1506)

nearest_par <- function(x, targets) {
  targets[which.min(abs(x - targets))]}

rlc.predictions <- rlc.predictions %>%
  dplyr::mutate(par_bin = vapply(par, nearest_par, numeric(1), targets = target_par))

rlc.predictions.pop <- rlc.predictions.pop %>%
   dplyr::mutate(par_bin = vapply(par, nearest_par, numeric(1), targets = target_par)) %>%
   dplyr::group_by(id, par_bin) %>%
   dplyr::summarize(
    fit.pop = mean(fit.pop),   # average predictions in the same bin
    .groups = "drop")
  
rlc.summary.pop <- rlc.predictions %>%
  dplyr::group_by(id, par_bin) %>%
  dplyr::summarize(
    etr_mean = mean(etr, na.rm = TRUE),
    etr_se   = sd(etr, na.rm = TRUE) / sqrt(dplyr::n()),
    n = dplyr::n(),
    .groups = "drop")

PGH_pop_fits <- ggplot(rlc.summary.pop, aes(x = par_bin, y = etr_mean, color = id)) +
  geom_point(size = 3, show.legend = FALSE) + # Averaged data
  geom_errorbar(aes(ymin = (etr_mean - etr_se),
                  ymax = (etr_mean + etr_se), width = 100)) +
  geom_line(data = rlc.predictions.pop, aes(y = fit.pop), linewidth = 1, alpha = 0.7) + # Fitted model
  geom_vline(data = vertical_lines, aes(xintercept = Ek, color = id), linewidth = 0.75, alpha = 0.7, 
             linetype = "longdash", show.legend = FALSE) +
  #geom_hline(data = horizontal_lines, aes(yintercept = ETRmax, color = id), size = 0.75, 
  ...
  facet_wrap(~id, labeller = depths, ncol = 3) +
...
leveneTest(alpha ~id, data = rlc.stats)
leveneTest(beta ~id, data = rlc.stats)

ETRmax_anova <- aov(ETRmax ~ id, data = rlc.stats)
Ek_anova <- aov(Ek ~ id, data = rlc.stats)
alpha_anova <- aov(alpha ~ id, data = rlc.stats)
beta_anova <- aov(beta ~ id, data = rlc.stats)
...
TukeyHSD(Ek_anova)
TukeyHSD(alpha_anova)

```

# Plot Depth Fits Figure
```{r}
rlcs_all <- ggplot(rlc.predictions, aes(x = par_bin, y = etr, color = id)) +
  geom_point(size = 3, alpha = 0.75, show.legend = FALSE) + # Observed data
  geom_line(data = rlc.predictions.pop, aes(y = fit.pop), size = 1.75) + # Fitted model
  geom_vline(data = vertical_lines, aes(xintercept = Ek, color = id), size = 1.5, 
             linetype = "dashed", show.legend = FALSE) +
  geom_hline(data = horizontal_lines, aes(yintercept = ETRmax, color = id), size = 1.5, 
             linetype = "dashed", show.legend = FALSE) +
  
  # Deep Labels
  geom_text(data = Deep_labels, aes(label = label_ek), size = 14, fontface = "bold", 
            x = 390, y = 1, show.legend = FALSE) +
  geom_text(data = Deep_labels, aes(label = label_ETRmax), size = 14, fontface = "bold", 
            x = 1330, y = 63, show.legend = FALSE) +

  # Midd Labels
  geom_text(data = Midd_labels, aes(label = label_ek), size = 14, fontface = "bold", 
            x = 510, y = 11, show.legend = FALSE) +
  geom_text(data = Midd_labels, aes(label = label_ETRmax), size = 14, fontface = "bold", 
            x = 1330, y = 87, show.legend = FALSE) +
  
  # Shall Labels
  geom_text(data = Shall_labels, aes(label = label_ek), size = 14, fontface = "bold", 
            x = 510, y = 18, show.legend = FALSE) +
  geom_text(data = Shall_labels, aes(label = label_ETRmax), size = 14, fontface = "bold", 
            x = 1330, y = 78, show.legend = FALSE) +
  ...
  theme(legend.position = "none",
    ...
    labs(x = "PAR (µmol photons m⁻² s⁻¹)" ,y ="ETR (µmol electrons m)"))

ggsave(
  filename = "Plots/rlcs/rlcs_all_depths.png", 
  plot = rlcs_all, 
  height = 6, 
  width = 10)
```
