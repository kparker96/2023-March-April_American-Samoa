...
files <- list.files(path ="./data/respirometry/middle/400/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/middle/400/check_plots/', col, "_400", ".pdf"))
    inspect(data_subset)
...
#### Photosynthesis Calculations ####
    ...
    # calc_rate()
    photo <- calc_rate(data_subset, from = 0, to = 18, by = 'time', plot = TRUE)
    pdf(paste0('data/respirometry/middle/400/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    pdf(paste0('data/respirometry/middle/400/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
library("writexl")
write_xlsx(final_combined, "data/respirometry/middle/final_rates/400_rates.xlsx")

write_csv(final_combined, "data/respirometry/middle/final_rates/400_rates.csv")
