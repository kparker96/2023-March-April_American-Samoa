...
files <- list.files(path ="./data/respirometry/deep/300/", pattern = "*_cleaned.txt", full.names = T)
...
df_clean <- df %>%
  ...
  select(Depth, LightIntensity, Time, Gen01, Gen02, Gen03, Gen04, Gen05, Gen06, Gen07, Gen08) 
...
df_ready <- df_clean %>%
  select(Time = time_in_minutes, Gen01, Gen02, Gen03, Gen04, Gen05, Gen06, Gen07, Gen08)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/deep/300/check_plots/', col, "_300", ".pdf"))
    inspect(data_subset)
...
```{r import surface area measurements}
frag_data <- read_excel("data/respirometry/Fragment_Surface_Area_and_Volume.xlsx") %>%
  filter(Depth == "Deep")

surface_area <- frag_data %>%
  select(Sample, Genotype, `SA(m2)`)

effective_volume <- frag_data %>%
  select(Sample, Genotype, effective_volume)
```
...
for (Geno in data_cols) {
    ...
    # Surface Area #
    area_value <- surface_area$`SA(m2)`[surface_area$Genotype == Geno]
    volume_value <- effective_volume$effective_volume[effective_volume$Genotype == Geno]
    
#### Photosynthesis Calculations ####
    
    # calc_rate()
    photo <- calc_rate(data_subset, from = 0, to = 20, by = 'time', plot = TRUE)
    pdf(paste0('data/respirometry/deep/300/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
    ...
    photo_final <- convert_rate(photo_adj,              # rate adjusted for background
                           ...
                           output.unit = "umol/min/m2", # output unit
                           volume = volume_value,         # effective volume of the respirometer (L)
                           area = area_value)            # area of sample (m2)
    
#### Respiration calculations ####
    
    # calc_rate()
    resp <- calc_rate(data_subset, from = 22, to = 40, plot = TRUE)
    pdf(paste0('data/respirometry/deep/300/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
final_combined <- rbind(master_resp_final, master_photo_final) %>%
  ...
  mutate(light = 300) # add in light intensity 

# Adjust file name for specific sample set 
library("writexl")
write_xlsx(final_combined, "data/respirometry/deep/final_rates/300_rates.xlsx")

write.csv(final_combined, "data/respirometry/deep/final_rates/300_rates.csv", row.names = FALSE)
...
ggplot(df_plot, aes(x = Time, y = o2, color = Geno)) + geom_point() + ylim(c(115,190)) + facet_wrap(~Geno, scales = "free")
```
