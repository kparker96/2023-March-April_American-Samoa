...
files <- list.files(path ="./data/respirometry/middle/500/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/middle/500/check_plots/', col, "_500", ".pdf"))
    inspect(data_subset)
...
#### Photosynthesis Calculations ####
    ...
    pdf(paste0('data/respirometry/middle/500/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    # calc_rate()
    resp <- calc_rate(data_subset, from = 22, to = 35, plot = TRUE)
    pdf(paste0('data/respirometry/middle/500/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
library("writexl")
write_xlsx(final_combined, "data/respirometry/middle/final_rates/500_rates.xlsx")

write_csv(final_combined, "data/respirometry/middle/final_rates/500_rates.csv")
...
ggplot(df_plot, aes(x = Time, y = o2, color = Geno)) + geom_point() + ylim(c(100,300)) + facet_wrap(~Geno)
```
