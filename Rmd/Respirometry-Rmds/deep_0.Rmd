...
files <- list.files(path ="./data/respirometry/deep/0/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/deep/0/check_plots/', col, "_0", ".pdf"))
    inspect(data_subset)
...
#### Photosynthesis Calculations ####
    ...
    pdf(paste0('data/respirometry/deep/0/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    # calc_rate()
    resp <- calc_rate(data_subset, from = 5, to = 20, plot = TRUE)
    pdf(paste0('data/respirometry/deep/0/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
final_combined <- rbind(master_resp_final, master_photo_final) %>%
  ...
  mutate(light = 0) # add in light intensity 
```
...
library("writexl")
write_xlsx(final_combined, "data/respirometry/deep/final_rates/0_rates.xlsx")

write.csv(final_combined, "data/respirometry/deep/final_rates/0_rates.csv", row.names = FALSE)
