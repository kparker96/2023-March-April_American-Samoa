...
files <- list.files(path ="./data/respirometry/deep/400/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/deep/400/check_plots/', col, "_400", ".pdf"))
    inspect(data_subset)
...
#### Photosynthesis Calculations ####
    ...
    pdf(paste0('data/respirometry/deep/400/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    # calc_rate()
    resp <- calc_rate(data_subset, from = 24, to = 35, plot = TRUE)
    pdf(paste0('data/respirometry/deep/400/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
final_combined <- rbind(master_resp_final, master_photo_final) %>%
  ...
  mutate(light = 400) # add in light intensity 

# Adjust file name for specific sample set 
library("writexl")
write_xlsx(final_combined, "data/respirometry/deep/final_rates/400_rates.xlsx")

write.csv(final_combined, "data/respirometry/deep/final_rates/400_rates.csv", row.names = FALSE)
