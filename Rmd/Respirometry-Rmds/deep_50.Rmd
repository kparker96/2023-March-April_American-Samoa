...
files <- list.files(path ="./data/Ch2/respirometry/deep/50/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/Ch2/respirometry/deep/50/check_plots/', col, "_50", ".pdf"))
    inspect(data_subset)
...
frag_data <- read_excel("data/Ch2/respirometry/Fragment_Surface_Area_and_Volume.xlsx") %>%
  filter(Depth == "Deep")
...
frag_data %>%
  select(Sample, Genotype, `Buoyant_Weight(g)`) %>%
  mutate(volume = `Buoyant_Weight(g)`/1.025)
```
...
#### Photosynthesis Calculations ####
    ...
    pdf(paste0('data/Ch2/respirometry/deep/50/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    pdf(paste0('data/Ch2/respirometry/deep/50/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
library("writexl")
write_xlsx(final_combined, "data/Ch2/respirometry/deep/final_rates/50_rates.xlsx")

write.csv(final_combined, "data/Ch2/respirometry/deep/final_rates/50_rates.csv", row.names = FALSE)
