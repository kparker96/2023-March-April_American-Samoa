...
files <- list.files(path ="./data/respirometry/deep/50/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/deep/50/check_plots/', col, "_50", ".pdf"))
    inspect(data_subset)
...
#### Photosynthesis Calculations ####
    ...
    pdf(paste0('data/respirometry/deep/50/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    # calc_rate()
    resp <- calc_rate(data_subset, from = 40, to = 50, plot = TRUE)
    pdf(paste0('data/respirometry/deep/50/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
library("writexl")
write_xlsx(final_combined, "data/respirometry/deep/final_rates/50_rates.xlsx")

write.csv(final_combined, "data/respirometry/deep/final_rates/50_rates.csv", row.names = FALSE)
