...
files <- list.files(path ="./data/respirometry/shallow/100/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/shallow/100/check_plots/', col, "_100", ".pdf"))
    inspect(data_subset)
...
#### Photosynthesis Calculations ####
    ...
    # calc_rate()
    photo <- calc_rate(data_subset, from = 5, to = 15, by = 'time', plot = TRUE)
    pdf(paste0('data/respirometry/shallow/100/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    pdf(paste0('data/respirometry/shallow/100/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
library("writexl")
write_xlsx(final_combined, "data/respirometry/shallow/final_rates/100_rates.xlsx")

write_csv(final_combined, "data/respirometry/shallow/final_rates/100_rates.csv")
