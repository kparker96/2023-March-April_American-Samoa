...
files <- list.files(path ="./data/respirometry/deep/100/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/deep/100/check_plots/', col, "_100", ".pdf"))
    inspect(data_subset)
...
#### Photosynthesis Calculations ####
    ...
    pdf(paste0('data/respirometry/deep/100/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    # calc_rate()
    resp <- calc_rate(data_subset, from = 24, to = 38, plot = TRUE)
    pdf(paste0('data/respirometry/deep/100/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
final_combined <- rbind(master_resp_final, master_photo_final) %>%
  ...
  mutate(light = 100) # add in light intensity 

# Adjust file name for specific sample set 
library("writexl")

write_xlsx(final_combined, "data/respirometry/deep/final_rates/100_rates.xlsx")

write.csv(final_combined, "data/respirometry/deep/final_rates/100_rates.csv", row.names = FALSE)
```
...
ggplot(df_plot, aes(x = Time, y = o2, color = Geno)) + geom_point() + facet_wrap(~Geno)
```
