...
files <- list.files(path ="./data/Ch2/respirometry/deep/100/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/Ch2/respirometry/deep/100/check_plots/', col, "_100", ".pdf"))
    inspect(data_subset)
...
surface_area <- read_excel("data/Ch2/respirometry/Fragment_Surface_Area.xlsx") %>%
  filter(Depth == "Deep")
...
#### Photosynthesis Calculations ####
    ...
    pdf(paste0('data/Ch2/respirometry/deep/100/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    pdf(paste0('data/Ch2/respirometry/deep/100/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
# Data Check ( Are any R2 < 0.8)
```{r low_r2_all}
low_r2_all <- full_join(
  master_photo %>% 
    filter(rsq < 0.8) %>% 
    select(Sample_name, rsq) %>% 
    rename(photo_r2 = rsq),
...
low_r2_all # Gen08 is blank chamber 
```
...
write_xlsx(final_combined, "data/Ch2/respirometry/deep/final_rates/100_rates.xlsx")

write.csv(final_combined, "data/Ch2/respirometry/deep/final_rates/100_rates.csv", row.names = FALSE)
```
