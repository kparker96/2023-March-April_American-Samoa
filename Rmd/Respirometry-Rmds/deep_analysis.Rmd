...
```{r file_list}
# Import files
file_list <- list.files(path = "data/Ch2/respirometry/deep/final_rates", pattern = "*.csv", full.names = T)
...
master_df <- file_list_compiled %>%
  ...
  mutate(net_photosynthesis = photosynthesis_raw + respiration) %>%
  mutate(gross_photosynthesis = photosynthesis_raw + abs(respiration))
...
no_light_meas <- master_df %>%
  ...
  select(Sample_name, respiration_intercept_b0, 
         respiration_slope_b1, respiration_rsq, respiration, light)
...
ggplot(avg_gross_photo_df, aes(x = light, y = mean_gross_photosynthesis)) +
