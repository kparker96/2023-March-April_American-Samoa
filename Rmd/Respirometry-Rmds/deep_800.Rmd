...
files <- list.files(path ="./data/respirometry/deep/800/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/deep/800/check_plots/', col, "_800", ".pdf"))
    inspect(data_subset)
...
#### Photosynthesis Calculations ####
    ...
    pdf(paste0('data/respirometry/deep/800/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
#### Respiration calculations ####
    ...
    pdf(paste0('data/respirometry/deep/800/full_plots/', Geno, "_resp.pdf"))
    print(resp)
    dev.off()
...
final_combined <- rbind(master_resp_final, master_photo_final) %>%
  ...
  mutate(light = 800) # add in light intensity 

# Adjust file name for specific sample set 
library("writexl")
write_xlsx(final_combined, "data/respirometry/deep/final_rates/800_rates.xlsx")

write.csv(final_combined, "data/respirometry/deep/final_rates/800_rates.csv", row.names = FALSE)
