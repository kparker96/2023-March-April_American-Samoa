...
files <- list.files(path ="./data/respirometry/sandbox/", pattern = "*_cleaned.txt", full.names = T)
...
for (col in data_cols) {
    ...
    pdf(paste0('data/respirometry/sandbox/check_plots/', col, "_sandbox", ".pdf"))
    inspect(data_subset)
...
for (Geno in data_cols) {
    ...
    pdf(paste0('data/respirometry/sandbox/full_plots/', Geno, "_resp.pdf"))
    plot(resp)
    dev.off()
...
for (Geno in data_cols) {
    ...
    pdf(paste0('data/respirometry/sandbox/full_plots/', Geno, "_photo.pdf"))
    plot(photo)
    dev.off()
...
library("writexl")
write_xlsx(final_combined, "data/respirometry/sandbox/final_rates/sandbox_rates.xlsx")
