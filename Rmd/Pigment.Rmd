...
labels <- c(
  "Chl_a_per_cm2" = "Chlorophyll~italic(a)",
  "Chl_c_per_cm2" = "Chlorophyll~italic(c)",
  "Perid_per_cm2"    = "Peridinin")

total_pigment <- ggplot(pigment_plot_per_cm2, aes(x = Depth, y = ug_cm2_pigment, fill = Depth)) +
  geom_boxplot() + 
  scale_fill_manual(values = c('darkblue', 'dodgerblue3', '#6BAED6'),
                    labels = c( "Deep", "Middle", "Shallow")) +
  facet_wrap(~Pigment, labeller = as_labeller(labels, label_parsed), scales = "free_y") +
  
  #scale_y_continuous(limits = c(0,1.25), expand = c(0,0)) +
  ylab(expression("Pigment" * " " *~(mu*g/cm^2))) +
  theme_bw() +
...
ggsave(
  filename = "Plots/Pigment/total_pigment.pdf",  # Name of the output file
  plot = total_pigment,                      # Specify the plot to save
  width = 5,                            # Width in inches
  height = 4                            # Height in inches
)

ggsave(
  filename = "Plots/Pigment/total_pigment.png",  # Name of the output file
  plot = total_pigment,                      # Specify the plot to save
  width = 5,                            # Width in inches
  height = 4                           # Height in inches
)

saveRDS(total_pigment, file = "Plots/Pigment/total_pigment.rds")
...
# Chl_a
anova_Chl_a <- aov(Chl_a_per_cm2 ~ Depth, data = pigment_stats)

summary(anova_Chl_a)
TukeyHSD(anova_Chl_a)

# Chl_c
anova_Chl_c <- aov(Chl_c_per_cm2 ~ Depth, data = pigment_stats)

summary(anova_Chl_c)
TukeyHSD(anova_Chl_c)

# Perid
anova_Perid <- aov(Perid_per_cm2 ~ Depth, data = pigment_stats)

summary(anova_Perid)
TukeyHSD(anova_Perid)

```

# Chlorophyll C:A ratios
```{r}
C_A_ratio <- pigment_plot_per_cm2 %>%
  pivot_wider(names_from = "Pigment",
              values_from = "ug_cm2_pigment") %>%
  mutate(ratio_CA = Chl_c_per_cm2/Chl_a_per_cm2)

C_A_ratio %>%
  group_by(Depth) %>%
  summarise(mean(ratio_CA))

shapiro.test(C_A_ratio$ratio_CA)
leveneTest(data = C_A_ratio, ratio_CA ~ Depth)

kruskal.test(ratio_CA ~ Depth, data = C_A_ratio)
```
